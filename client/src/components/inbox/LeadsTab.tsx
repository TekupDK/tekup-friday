...
function getStatusColor(status: string): string {
  switch (status) {
    case "won": return "bg-green-100 text-green-800 border-green-300 dark:bg-green-900/30 dark:text-green-400";
    case "proposal": return "bg-blue-100 text-blue-800 border-blue-300 dark:bg-blue-900/30 dark:text-blue-400";
    case "qualified": return "bg-yellow-100 text-yellow-800 border-yellow-300 dark:bg-yellow-900/30 dark:text-yellow-400";
    case "contacted": return "bg-orange-100 text-orange-800 border-orange-300 dark:bg-orange-900/30 dark:text-orange-400";
    case "new": return "bg-gray-100 text-gray-800 border-gray-300 dark:bg-gray-800 dark:text-gray-400";
    case "lost": return "bg-red-100 text-red-800 border-red-300 dark:bg-red-900/30 dark:text-red-400";
...
export default function LeadsTab() {
  ...
  const { data: profiles } = trpc.customer.listProfiles.useQuery(undefined, { staleTime: 60_000 });
  ...
  console.log("🎯 [LeadsTab] Leads data:", leads ? `${leads.length} leads` : "Loading...");
  ...
  if (leads) {
    ...
    if (profiles && leads) {
      ...
      console.log("🎯 [LeadsTab] Sync summary:", { billyLinked: billyLeads.length, inboxLinked: inboxLeads.length });
      console.log("🎯 [LeadsTab] Billy-linked leads:", billyLeads.map(l => ({ id: l.id, name: l.name, email: l.email, score: l.score, status: l.status })));
      console.log("🎯 [LeadsTab] Inbox-linked leads:", inboxLeads.map(l => ({ id: l.id, name: l.name, email: l.email, score: l.score, status: l.status })));
    }
    ...
    if (searchQuery || statusFilter !== "all" || sourceFilter !== "all") {
      ...
      console.log("🎯 [LeadsTab] Filtered results:", `${filteredLeads.length} leads`);
    }
  }
  ...
  const stats = leads ? (() => {
    ...
    return { total, new: newCount, hot, won, lost, avgScore, billyLinked, inboxLinked };
  })() : null;
    ...
    <>
      ...
      {stats && (
        <div className="mb-4 grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-3">
          ...
          <Card className="p-3">
            <div className="text-xs text-muted-foreground mb-1">Billy Linked</div>
            <div className="text-2xl font-bold text-emerald-600">{stats.billyLinked}</div>
          </Card>
          <Card className="p-3">
            <div className="text-xs text-muted-foreground mb-1">Inbox Linked</div>
            <div className="text-2xl font-bold text-blue-600">{stats.inboxLinked}</div>
          </Card>
        </div>
      )}
      ...
      <ScrollArea className="h-[calc(100vh-28rem)]">
        ...
        {filteredLeads && filteredLeads.length > 0 ? (
          filteredLeads.map((lead) => (
            <Card key={lead.id} className={`p-4 border-l-4 ${getScoreColor(lead.score)}`}>
              ...
              <div className="flex items-center gap-2">
                ...
                {(() => {
                  ...
                  const hasInbox = lead.source === 'gmail' || (p && (p.emailCount || 0) > 0);
                  return (
                    <>
                      {hasBilly && (
                        <Badge variant="outline" className="border-green-300 text-green-700 dark:text-green-400">Billy</Badge>
                      )}
                      {hasInbox && (
                        <Badge variant="outline" className="border-blue-300 text-blue-700 dark:text-blue-400">Inbox</Badge>
                      )}
                    </>
                  );
                })()}
          ...
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p>{searchQuery ? "Ingen leads matcher din søgning" : "No leads found"}</p>
          </div>
        )}
        </div>
      </ScrollArea>
